...
module.exports = async function ({
    ...
    const { deployer, owner, helper, vesting1, vesting2, vesting3, vesting4 } =
    ...
    console.log(
        'JAKANTMasterDistributor deployed to:',
    ...
    const MproMasterDistributorFactory = await ethers.getContractFactory(
        'contracts/MPROMasterDistributor.sol:JAKANTMasterDistributor',
    )
    const MproMasterDistributor = (await MproMasterDistributorFactory.attach(
        mproMasterDistributor.address,
    )) as JAKANTMasterDistributor
    ...
    const mproToken = await deploy('JAKANTToken', {
        ...
        args: [
            TOKEN_NAME,
            TOKEN_SYMBOL,
            [vesting1, vesting2, vesting3, vesting4], // Premint addresses
            [
    ...
    await MproMasterDistributor.connect(helperSigner).setJAKANTToken(
        mproToken.address
    )
    // // Grant role master distributor to mproMasterDistributor
    await MproMasterDistributor.connect(helperSigner).grantRole(
        await MproMasterDistributor.JAKANT_MASTER_DISTRIBUTOR_ROLE(),
        MPRO_MASTER_DISTRIBUTOR,
    )
