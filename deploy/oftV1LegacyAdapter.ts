...
// npx hardhat deploy --tags OFTV1LegacyAdapter --network sepolia
// npx hardhat deploy --tags OFTV1LegacyAdapter --network base-sepolia
...
module.exports = async function ({
	...
	const oftV1Adapter = await deploy('OFTV1LegacyAdapter', {
		from: deployer,
		args: [oftPlainToken.address, lzEndpointAddress, owner],
		log: true,
		waitConfirmations: 1,
		skipIfAlreadyDeployed: true,
		contract: 'contracts/mocks/OFTV1LegacyAdapter.sol:OFTV1Adapter',
	})

	console.log('OFTV1LegacyAdapter deployed to:', oftV1Adapter)

	await verifyContractWithRetry(
		'contracts/mocks/OFTV1LegacyAdapter.sol:OFTV1Adapter',
		oftV1Adapter.address,
		oftV1Adapter.args,
	)
}

module.exports.tags = ['OFTV1LegacyAdapter']
