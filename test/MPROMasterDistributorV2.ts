...
describe("JAKANTMasterDistributor", () => {
  ...
  let JAKANT_master_distributor_role: HardhatEthersSigner;
  ...
  beforeEach(async () => {
    [
      JAKANT_master_distributor_role,
      lister_role,
    ...
    const MasterDistributorFactory = (await ethers.getContractFactory(
      "contracts/MPROMasterDistributor.sol:JAKANTMasterDistributor"
    )) as JAKANTMasterDistributor__factory;
    ...
    await mproMasterDistributor
      .connect(owner)
      .grantRole(
        await mproMasterDistributor.JAKANT_MASTER_DISTRIBUTOR_ROLE(),
        JAKANT_master_distributor_role.address
      );
    await mproMasterDistributor
      .connect(owner)
      .grantRole(
    ...
    const JAKANTToken: JAKANTToken__factory = (await ethers.getContractFactory(
      "contracts/MPRO.sol:JAKANTToken"
    )) as JAKANTToken__factory;
    ...
    it("Should not revert with correct parameters", async () => {
      ...
      await expect(
        mproMasterDistributor
          .connect(JAKANT_master_distributor_role)
  ...
  describe("setJAKANTToken function", () => {
    it("Should revert when caller is not the owner", async () => {
      await expect(
        mproMasterDistributor.connect(user).setJAKANTToken(mproToken.target)
      ).to.be.revertedWith(`Ownable: caller is not the owner`);
    });

    it("Should revert when token is already set", async () => {
      await expect(
        mproMasterDistributor.connect(owner).setJAKANTToken(mproToken.target)
      ).to.be.revertedWith(
  ...
  describe("setDistributorTimeAdministratorRoleManager", () => {
    ...
    it("Should properly burn the role and than revert when trying to assign the role", async () => {
      ...
      ).to.be.revertedWith("JAKANTMasterDistributor: Role is already burned");
    });
  ...
  describe("grantRole function", () => {
    ...
    it("Should revert when trying to grant the roles to address zero", async () => {
      ...
      ).to.be.revertedWith("JAKANTMasterDistributor: Action on address zero");
      await expect(
        mproMasterDistributor
          .connect(lister_role)
  ...
  describe("mintAllowed function", () => {
    ...
    it("Should return false if called by address zero", async () => {
      ...
      ).to.be.revertedWith("JAKANTMasterDistributor: Distributor only");
    });
  });
