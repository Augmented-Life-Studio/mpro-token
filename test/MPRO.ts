...
import {
  JAKANTToken,
  JAKANTToken__factory,
  JAKANTMasterDistributor,
  JAKANTMasterDistributor__factory,
...
describe("JAKANTToken", function () {
  ...
  beforeEach(async function () {
    ...
    await masterDistributor
      .connect(owner)
      .grantRole(
        await masterDistributor.JAKANT_MASTER_DISTRIBUTOR_ROLE(),
        owner.address
      );
    await masterDistributor
      .connect(owner)
    ...
    const JAKANTTokenFactory = (await ethers.getContractFactory(
  ...
  describe.only("mint function", function () {
    it("Should fail to mint tokens when called by non-distributor", async function () {
      await expect(
        mproToken.connect(addr1).mint(addr2.address, ethers.parseEther("50"))
      ).to.be.revertedWith("JAKANTMasterDistributor: Distributor only");
    });
  });

  describe.only("burn function", function () {
