...
module.exports = async function (taskArgs: any, hre: any) {
	...
	// get local contract
	const localContractInstance = (await hre.ethers.getContract(
		localContract,
	)) as MPRO
	...
	try {
		let tx = await (
			await localContractInstance
				.connect(fromSigner)
				.send(sendParams, [nativeFee, zero] as any, taskArgs.fromAddress, {
					value: nativeFee,
				})
		).wait()
		...
		console.log(
			`✅ Message Sent [${hre.network.name}] send() to OFT @ LZ chainId[${remoteChainId}] token:[${remoteContract}]`,
		)
		console.log(` tx: ${tx?.hash}`)
		console.log(
			`* check your address [${toAddress}] on the destination chain, in the ERC20 transaction tab !"`,
		)
	} catch (error) {
		console.log('====================================')
		console.log(error)
